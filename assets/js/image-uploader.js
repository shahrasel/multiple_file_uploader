    ...
    e.fn.imageUploader = function(t) {
        ...
        let o = function() {
            ...
            g = function(e) {
                ...
                return !(a.settings.extensions.indexOf(e.name.replace(new RegExp("^.*\\."), ".")) < 0) || ($('#err').html(`The file "${e.name}" does not match with the accepted file extensions: "${a.settings.extensions.join('", "')}"`).fadeIn(), !1)
            },
            c = function(e) {
                $("#err").css('display','none');
                $("#preview").css('display','none');
                return !(a.settings.mimes.indexOf(e.type) < 0) || ($('#err').html(`The file "${e.name}" does not match with the accepted mime types: "${a.settings.mimes.join('", "')}"`).fadeIn(), !1)
            },
            f = function(e) {
                $("#err").css('display','none');
                $("#preview").css('display','none');
                return !(e.size > a.settings.maxSize) || ($('#err').html(`The file "${e.name}" exceeds the maximum size of ${a.settings.maxSize/1024/1024}Mb`).fadeIn(), !1)
            },
            m = function(e, t) {
                $("#err").css('display','none');
                $("#preview").css('display','none');
                return !(e + s.items.length + a.settings.preloaded.length >= a.settings.maxFiles) || ($('#err').html(`The file "${t.name}" could not be added because the limit of ${a.settings.maxFiles} files was reached`).fadeIn(), !1)
            },
            u = function(t, n) {
                ...
                e(n).each(function(e, t) {
                    s.items.add(t), i.append(l(t.type, URL.createObjectURL(t), s.items.length - 1), !1)
